  ...
  return (
    <main className="min-h-screen p-8 md:p-16 max-w-5xl mx-auto font-mulish leading-[1.8em]">
      <header className="mb-12 text-center">
        <h1 className="text-4xl md:text-5xl font-bold mb-4 font-poppins text-maroon">
          Mijn Zorgkompas
        </h1>
        <p className="text-xl text-gray-600">
          NL-uitleg bij ouderenzorg en ondersteuning in Frankrijk
        </p>
      </header>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {zorgData.categories.map((cat) => (
            <button
              ...
              className="p-8 border-2 border-maroon/20 rounded-2xl hover:border-maroon hover:bg-softYellow transition-all text-left"
            >
              <h2 className="text-2xl font-bold mb-2 font-poppins text-maroon">
                {cat.label_nl}
              </h2>
              <p className="italic text-maroon/70 mb-4">
                {cat.label_fr}
              </p>
      ...
      {step === 'resultaten' && selectedCategory && (
        <section className="space-y-12 animate-in fade-in duration-500">
          ...
          <div className="bg-softYellow/30 p-8 rounded-3xl border border-maroon/10">
            <h2 className="text-3xl font-bold mb-6 font-poppins text-maroon">
              Uw gids voor: {selectedCategory.label_nl}
            </h2>

            <div className="space-y-6 mb-10">
              <h3 className="text-2xl font-semibold border-b border-maroon/10 pb-2 font-poppins text-maroon">
              ...
              <div className="grid gap-4">
                {selectedCategory.related_definitions.map((defKey) => {
                  ...
                  return (
                    ...
                    >
                      <h4 className="text-xl font-bold mb-1 font-poppins text-maroon">
                        {def.term_nl} ({def.term_fr})
                      </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {selectedCategory.related_contacts.map((contactKey) => {
                  ...
                  return (
                    <div
                      key={contactKey}
                      className="bg-white p-6 rounded-xl border-l-8 border-maroon shadow-md"
                    >
                      <h4 className="text-xl font-bold mb-1 font-poppins text-maroon">
                        {contact.naam}
                      </h4>
          ...
          <div className="bg-maroon text-white p-10 rounded-3xl shadow-xl">
            <h3 className="text-2xl font-bold mb-4 font-poppins text-center md:text-left">
              Vind lokale ondersteuning (CLIC / Point d’information local)
            </h3>

            <p className="mb-8 opacity-90 text-lg">
              In Frankrijk wordt ouderenzorg lokaal gecoördineerd. Via een Point
              d’information local (zoals CLIC) krijgt u uitleg over zorg,
              ondersteuning en aanvragen in uw regio.
            </p>
